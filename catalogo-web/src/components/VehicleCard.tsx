...
function getPrimaryUrl(v: Vehicle): string | null {
  ...
  const url =
    base?.formats?.medium?.url ?? base?.formats?.small?.url ?? base?.url;
...
export default function VehicleCard({
  ...
  const wsp = whatsapp
    ? wspLink(whatsapp, `Hola, me interesa el ${a?.title ?? "vehículo"}`)
    : null;
  ...
  return (
    <article
      className={`group rounded-2xl border border-white/10 bg-[#121212] shadow-sm transition hover:shadow-xl hover:border-white/20 ${className}`}
    >
      {/* Imagen */}
      ...
      <div className="p-4 md:p-5">
        ...
        <p className="mt-0.5 text-sm text-neutral-400">
          {a?.location ?? "Ubicación no especificada"}
        </p>

        <div className="mt-3 flex items-center justify-between">
          <div className="text-[1.35rem] font-extrabold text-red-500">
            {priceText}
          </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-[13px] text-neutral-300">
          <Item
            icon={
              ...
              >
                <path
                  d="M3 12h18M7 12a5 5 0 1 1 10 0"
                  stroke="currentColor"
                  strokeWidth="1.6"
                  strokeLinecap="round"
                />
          ...
          >
            {a?.mileageKm
              ? `${Number(a.mileageKm).toLocaleString("es-CL")} km`
              : "—"}
          </Item>
          ...
          >
            {a?.transmission
              ? getTransmissionLabel(String(a.transmission))
              : "—"}
          </Item>
