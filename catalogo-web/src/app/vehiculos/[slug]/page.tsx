...
function StrapiBlocks({
  blocks,
}: {
  blocks: Vehicle["description"] | undefined;
}) {
  ...
  return blocks.map((block, index) => {
    ...
    if (b.type === "paragraph" && Array.isArray(b.children)) {
      return (
        <p
          key={index}
          className="mt-2 text-neutral-300 text-sm whitespace-pre-wrap"
        >
...
export default function VehiclePage() {
    ...
    return (
      <div className="bg-[#0A0A0A] text-white min-h-screen flex items-center justify-center">
        <div className="text-center p-8 rounded-lg bg-[#121212] border border-white/10">
          ...
          <p className="text-neutral-300">Por favor, espera un momento.</p>
        </div>
      </div>
    );
  }
  ...
  const mainPhotoUrl = absUrl(
    flattenAttributes(vehicle.primaryPhoto?.data)?.url ?? "",
  );
  const galleryUrls =
  ...
  const wsp = wspLink(
    staticWhatsappNumber,
    `Hola, me interesa el ${vehicle.title}`,
  );
  ...
  const renderThumbs = (children: React.ReactNode[]) => {
    return children.map((child, index) => {
      ...
      return (
        <div
          key={index}
          className={`thumb ${selectedItem === index ? "selected" : ""}`}
        >
          <Image
            src={image}
            alt={`thumbnail ${index}`}
              ...
              {allImages.length > 0 ? (
                ...
                >
                  {allImages.map((url, index) => (
                    <div
                      key={index}
                      className="relative aspect-[16/10] w-full bg-[#121212]"
                    >
                      <Image
                        src={url}
